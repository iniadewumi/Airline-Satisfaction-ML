...
class Training:
    ...
    def rescale_and_split(self):    
        ...
        scaled_inp[unscaled_X.columns] = pd.DataFrame(StandardScaler().fit_transform(unscaled_X))
        scaled_inp["Satisfaction"] = self.inp["Satisfaction"]

        self.x_train, self.x_test, self.y_train, self.y_test =  train_test_split(scaled_inp[unscaled_X.columns], scaled_inp["Satisfaction"] , test_size=0.2, random_state = 21)
        self.train_classif()
        
    def train_classif(self):
        classifiers = {"RandomForest": RandomForestClassifier(), "KNN": KNeighborsClassifier(n_neighbors=15), "SVM": svm.SVC()}
        ...
        conf_mat = confusion_matrix(self.y_test, self.pred)
        print(conf_mat)
    ...
    def init_nn(self):
        inp = input("Enter layers as tuple for NeuralNetwork, leave blank for default: ")
        layers = eval(inp) if inp else (50,30,20,5)
        if type(layers)!=tuple:
            raise Exception("Invalid Layers! Please retry")

        return MLPClassifier(activation="logistic", max_iter=300, hidden_layer_sizes=layers)
